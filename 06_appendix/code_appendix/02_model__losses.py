...
def intensity_soft_target(y_reg, tau=0.3):
    ...
    tau 越小越接近硬标签；tau->inf 退化为"只区分中性/非中性"。
    """
    y = y_reg
...
def emd2_loss(logits, q):
    """EMD^2（有序）惩罚：累计分布的平方差，使"跨类错误"代价 > "邻类错误"。"""
...
def mag_level(y_reg, n_levels=10):
    """|y| -> 等级 k in [0, n_levels-1]（|y| 的离散档位，步长 1/3 -> n_levels=10）。"""
...
def mag_consistency(mu, logits_mag, n_levels=10):
    ...
    exp_k = p.sum(dim=1)                              # E[k] = Sigma_j P(k>j)
...
def cls_D_loss(logits, y_cls, y_reg, cls_weight=None, label_smoothing=0.0,
    ...
    D2  lam_emd>0  ：有序 EMD^2 项（内建序结构；实测 84% 错误只是邻类错）
    D3  weak_alpha>0：按 w = 1 + alpha.(1-|y|/3) 给**弱标注样本**加权
...
def consistency_loss(mu, logits):
    ...
    即“极性 == 强度符号”是硬约束。因此 tanh(mu) 与分类头的期望极性 E[cls]=P(pos)-P(neg)
    应当一致；该正则把回归头与分类头对齐。
    """
    p = torch.softmax(logits, dim=-1)
    e = p[:, 2] - p[:, 0]                    # in[-1,1]
...
def compute_metrics(y_true_reg, y_pred_reg, y_true_cls, theta=0.0, y_pred_cls=None):
    ...
    # 极性判别有两套口径：
    #   (A) theta 阈值口径：|y_hat| <= theta 判为 Neutral（theta 在 valid 上按 Macro-F1 选出）
