...
内建自查断言（任一不过即报错退出）：
  ...
  2. pred_label / pred_score 与 submission/pred_att4.csv 逐位一致（同一骨干、同一 theta）；
  3. 解释数值有限、pi 和为 1、时间重要性非负；
...
def main():
    ...
    print("[DATA] att4 N=%d  (ckpt theta 均值 %.3f，提交用 theta=%.3f)" % (N, theta_ck, a.theta))
    ...
    for i, sid in enumerate(ids):
        ...
        m = meta.get(sid, {})
        # 断言：pi 归一化与时间重要性合法性
        assert abs(float(pi[i].sum()) - 1.0) < 1e-6, "pi 未归一化：%s" % sid
    ...
    print("[SAVE] %s（%d 行 x %d 列）" % (os.path.relpath(out_csv, ROOT), len(df), df.shape[1]))
