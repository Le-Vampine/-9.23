...
数学定义（论文 section5.3 用）：
    v(S) 为只用模态子集 S 时模型的目标标量；模态 m 的 Shapley 值
        phi_m = sum_{S subset M\{m}} |S|!(n-|S|-1)!/n! . [v(S|{m}) - v(S)],  n=3
    精确性检验：sum_m phi_m = v(M) - v({})。

本脚本一次算完 4 个口径组合（保证可比、避免重复前向）：
    target  in {mu(强度,主), p_pol = p_pos - p_neg(极性,对照)}
    semantic in {miss(置缺失指示,主), pad(整模态不可见,对照)}
并输出两套归一化（ReLU 归一 / softmax|phi|）供后续按验证集"模态忠实性"择优。

两类效用：
  * 样本级：v(S) 取该样本自身的预测 -> 逐样本 phi（写入附件4 解释 CSV）；
...
def main():
    ...
    for target, semantic in combos:
        ...
        print("  [%-10s] phi 均值 文/语/视 = %+0.4f / %+0.4f / %+0.4f ; 精确性残差 max=%.2e"
    ...
    if a.with_global:
        ...
        print("      phi(-MAE) = %s" % report["global"]["phi_negmae"])
        print("      phi(MacroF1) = %s" % report["global"]["phi_macrof1"])
