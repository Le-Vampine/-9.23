...
0. **Bundled install (VS Code / Copilot / any agent that loads a single skill
   folder):** `OTROOT` is the directory that contains *this* `SKILL.md`. The whole
   library is shipped alongside it — `catalog.json`, `meta.schema.json`,
   `templates/`, `icons/`, `reference/`, `examples/`, `tools/`, `docs/` — so
   `OTROOT` = this skill folder. Confirm once with `OTROOT/catalog.json`.
...
2. Otherwise (cloned repo / another agent with the original repo layout), `OTROOT` is
   the OpenTikZ repo root — the directory two levels above this `SKILL.md`
