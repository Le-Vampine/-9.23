# -*- coding: utf-8 -*-
r"""S4-a：三源时间重要性的归一化与融合（γ 在验证集上按与遮挡源的一致性标定）。
...
融合：先对每个样本、每个模态在**有效位内**做 min-max 归一化，再线性加权
      w = (γ1·w̃_att + γ2·w̃_ig + γ3·w̃_occ) / (γ1+γ2+γ3)，最后再归一化一次。
γ 的标定：在单纯形上枚举（**约束每个 γ ≥ 0.1**，避免退化为"只留遮挡源"这种平凡解），
...
def main():
    ...
    print("  [γ] 单纯形枚举（每源 ≥ %.1f），以与遮挡源的一致性为准则：" % 0.1)
    rows = []
    ...
    for g, ag in rows_sorted[:5]:
        print("      γ=(%.1f, %.1f, %.1f)  一致性 ρ=%.4f" % (*g, ag))
    g_best = rows_sorted[0][0]
    g_eq = (1 / 3, 1 / 3, 1 / 3)
    W_best = fuse(Ws, g_best)
    W_eq = fuse(Ws, g_eq)
    print("  [γ] 选定 γ_att/γ_ig/γ_occ = (%.2f, %.2f, %.2f)（等权对照 ρ=%.4f）"
