...
设计（论文 section5.3"可解释性模型的网络结构、目标函数、训练方案与关键参数"）：
  骨干**完全冻结**并保持预测通路不变；证据头挂在其**编码器输出**（逐模态 50x d 表示）上，
  只用于产生解释：s^m_k = softmax_k(w_m^T tanh(W_m h^m_k + b_m))，
  模态作用度 pi = softmax_m(u^T tanh(V h-^m))，h-^m = Sigma_k s^m_k h^m_k。

目标函数（全部可微；用"软删除/软保留"把忠实性写成可训练的损失）：
  L = alpha_f [ -(t(X) - t(X; miss=s))^2 + (t(X) - t(X; miss=1-s))^2 ]
      + alpha_1 Sigma_m ||s^m||_1 + alpha_2 Sigma_m TV(s^m) + alpha_3 Sigma_m (pi_m - pi_m^Shapley)^2
...
训练方案：只训证据头（骨干冻结、eval 模式），AdamW，lr 1e-3，30 轮，按验证集
          "忠实性 - 充分性偏差"加权目标早停；关键参数 alpha 在验证集上网格择优（默认给定值）。
